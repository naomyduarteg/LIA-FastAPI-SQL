...
@router.post("/", response_model=User, response_description="Create a user")
...
@router.get("/", response_model=list[User], response_description="Get list of users")
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud_users.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/{user_id}", response_model=User, response_description="Get user by id")
